...
def basic_cnn_upscaler_cifar(x_train, y_train, x_test, y_test, epochs=5,
                            retrain=False, model_filename="basic_cnn_upscaler_cifar.h5"):
...
def basic_cnn_downscaler_cifar(x_train, y_train, x_test, y_test, epochs=5,
                            retrain=False, model_filename="basic_cnn_downscaler_cifar.h5"):
    if os.path.isfile(model_filename) and not retrain:
        upscaler = load_model(model_filename)
        upscaler.summary()
    else:
...
def basic_dense_upscaler_cifar(x_train, y_train, x_test, y_test, epochs=5,
                            retrain=False, model_filename="basic_dense_upscaler_cifar.h5"):
    if os.path.isfile(model_filename) and not retrain:
        upscaler = load_model(model_filename)
        upscaler.summary()
    else:
        ...
        upscaler = Sequential([
            ...
            Dense((3 * 16 * 32), activation='relu'),
            Dense((3 * 32 * 32), activation='sigmoid'),
...
def basic_dense_downscaler_cifar(x_train, y_train, x_test, y_test, epochs=5,
                            retrain=False, model_filename="basic_dense_donwscaler_cifar.h5"):
    if os.path.isfile(model_filename) and not retrain:
        upscaler = load_model(model_filename)
        upscaler.summary()
    else:
        ...
        upscaler = Sequential([
            Flatten(input_shape=input_image_dim),
            Dense((3 * 16 * 32), activation='relu'),
            Dense((3 * 16 * 16), activation='sigmoid'),
            Reshape(target_shape = output_image_dim),
        ])
...
def basic_dense_autoencoder_cifar(x_train, y_train, x_test, y_test, epochs=5,
                            retrain=False, model_filename="basic_dense_autoencoder_cifar.h5"):
    if os.path.isfile(model_filename) and not retrain:
        upscaler = load_model(model_filename)
        upscaler.summary()
    else:
